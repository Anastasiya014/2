...
namespace LibMas
{
    public class Class1
    {
        ...
        //Заполнение массива
        public static void Fill( int count, out int[] mas)
        {
        ...
        public static void FillWithout_0(int count, out int[] mas)
        {
            //задаем массиву размерность
            mas = new int[count];
            ...
            for (int i = 0; i < mas.Length; i++)
            {
                //создаём элемент массива до тех пор пока он не равен 0
                while (mas[i] == 0)
                {
                    mas[i] = rnd.Next(-10, 10);
                }
        ...
        public static void Savemas(int[] mas)
        {
            ...
            if (save.ShowDialog() == true) //Если мы выбрали файл
            {
